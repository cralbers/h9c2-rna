...
qlf.CABvsDMSO <- glmQLFTest(fit, contrast=contr.matrix[,"CABvsDMSO"])
qlf.LENvsDMSO <- glmQLFTest(fit, contrast=contr.matrix[,"LENvsDMSO"])
qlf.CABvsLEN <- glmQLFTest(fit, contrast=contr.matrix[,"CABvsLEN"])

summary(decideTests(qlf.LENvsDMSO))

plotMD(qlf.CABvsDMSO)





go <- goana(qlf.CABvsDMSO)
...
# # subset large DGEList into 3 separate ones
...
con1 <- makeContrasts(CAB - DMSO, levels=design)
...
ddsds = as.DESeqDataSet(x)
ddsds$group <- relevel(ddsds$group, ref="DMSO")
...
res_cdLFC <- lfcShrink(dds, coef="group_CAB_vs_DMSO", type="apeglm")
res_cd<-results(dds, contrast=c("group", "CAB", "DMSO"))

res_ldLFC <- lfcShrink(dds, coef="group_LEN_vs_DMSO", type="apeglm")
res_ld<-results(dds, contrast=c("group", "LEN", "DMSO"))

res_clLFC <- lfcShrink(dds, coef="group_CAB_vs_LEN", type="apeglm")
res_cl<-results(dds, contrast=c("group", "CAB", "LEN"))

plotMA(res_cdLFC)


# write.csv(res_ld, "LENvsDMSO.csv")

DEreport(res_ld, genes=)
...
resdata_cd <- get_resdata(res_cd, dds)
resdata_ld <- get_resdata(res_ld, dds)
resdata_cl <- get_resdata(res_cl, dds)
...
ggplot(resdata_ld,aes(x=log2FC,y=-log(padj)))+
  geom_point(alpha=0.5)+labs(title=paste0(comparison),subtitle=paste0(mark))+
  xlim(-10,10)+
  #ylim(0,60)+
  geom_text(aes(label=paste0(nrow(subset(resdata_ld,log2FC > 1 & padj < 0.05))),x=Inf,y=Inf,vjust=2,hjust=2))+
  geom_text(aes(label=paste0(nrow(subset(resdata_ld,log2FC < (-1) & padj < 0.05))),x=-10,y=Inf,vjust=2))+
  theme_classic()

EnhancedVolcano(res_ldLFC,
                lab = rownames(res_ldLFC),
                x = 'log2FoldChange',
                y = 'pvalue')

glimmaMA(dds) # i am confusion

glimmaVolcano(dds, main="what is this comparing")
