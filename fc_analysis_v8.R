...
#   annots <- select(org.Rn.eg.db, keys=x$genes$GeneID, columns=c("REFSEQ", "ENTREZID", "ENSEMBL", "SYMBOL"), keytype="ENSEMBL")
#   m <- match(x$genes$GeneID, annots$ENSEMBL)
#   x$genes$Symbol <- annots$SYMBOL[m]
# }

# annots <- select(org.Rn.eg.db, keys=et.CABvsDMSO$genes$GeneID, columns=c("REFSEQ", "ENTREZID", "ENSEMBL", "SYMBOL"), keytype="ENSEMBL")

# add_symbol(et.CABvsDMSO)
...
EnhancedVolcano(data.frame(et.CABvsDMSO),
                lab = et.CABvsDMSO$genes$Symbol,
                selectLab=c("Nfe2l2"),
                x = 'logFC',
                y = 'PValue',
                title = 'CAB vs DMSO',
